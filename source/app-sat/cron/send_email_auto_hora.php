<?php
    ...
    $lojaBloqueio              = $rowAlertas['loja_bloqueio'];
    ...
    if($rowVerEmailAtivo['total_registros'] >= 1){
        ...
        $sqlListEmailExceptionAlerta = "SELECT email FROM cn_email_alarmes_exception WHERE status_email = 1 AND email_verif = 1";
        $queryListEmailExceptionAlerta = mysqli_query($conn,$sqlListEmailExceptionAlerta);
        while($rowListEmailExceptionAlerta = mysqli_fetch_array($queryListEmailExceptionAlerta)){
            $objListEmailExceptionAlerta[] = (object)$rowListEmailExceptionAlerta;
        }
        ...
        if($rowCountBDSAT['total_registros'] >= 1){
            ...
            while($rowBDSAT = mysqli_fetch_array($queryBDSAT)){
                ...
                if($cuponsAcumulados == 1){
                    ...
                    $rowSendEmail = mysqli_fetch_assoc($querySendEmail);
                    if ($rowSendEmail['send_email'] == 0) {
                        $sqlVerificaSATCuponsAcumulados = "SELECT COUNT(sat) AS existe_sat, sat, loja, caixa, numeros_cfes_memoria, n_dias FROM cn_hml_sat_cupons_memoria WHERE sat = '$sat'";
                        ...
                        if ($rowVerificaSATCuponsAcumulados['existe_sat'] >= 1) {
                            if ($rowVerificaSATCuponsAcumulados['numeros_cfes_memoria'] >= $nCuponsAcumulados && $rowVerificaSATCuponsAcumulados['n_dias'] >= $nDiasCuponsAcumulados) {
                ...
                if($lojaBloqueio == 1){
                    $sendEmail = "SELECT COUNT(sat) AS send_email FROM tb_send_email_auto WHERE motivo_email LIKE 'ESTADO_BLOCK_LOJA' AND sat = '$sat' AND DATE_FORMAT( data_hora_envio, '%Y-%m-%d' ) =  DATE_FORMAT(NOW(), '%Y-%m-%d')";
                    $querySendEmail = mysqli_query($conn,$sendEmail);
                    $rowSendEmail = mysqli_fetch_assoc($querySendEmail);
                    if ($rowSendEmail['send_email'] == 0) {
                        $sqlVerificaSATLojaBlock = "SELECT COUNT(sat) AS existe_sat, sat, loja, caixa, status_wan, disco_usado, numeros_cfes_memoria, data_hora_transm_sefaz, data_hora_comun_sefaz FROM cn_hml_dados_sat_group_lojas_s_comunicar_sefaz WHERE sat = '$sat'";
                        $queryVerificaSATLojaBlock = mysqli_query($conn, $sqlVerificaSATLojaBlock);
                        $rowVerificaSATLojaBlock = mysqli_fetch_assoc($queryVerificaSATLojaBlock);
                        $shop = $rowVerificaSATLojaBlock['loja'];
                        $pos = $rowVerificaSATLojaBlock['caixa'];
                        if ($rowVerificaSATLojaBlock['existe_sat'] >= 1) {
                            $objSATLojaBlock[] = (object) $rowVerificaSATLojaBlock;
                        }else{
                            $objSATLojaBlockI[] = (object) $rowVerificaSATLojaBlock;
                        }
                    }
                }
            }
            ...
            unset($msgLog);
            if(isset($objSATLojaBlockI)){
                $msgLog = "SATs ";
                foreach($objSATLojaBlockI as $key=>$val){
                    $msgLog = $msgLog . "[" . $objSATLojaBlockI[$key]->sat . "]";
                }
                require_once("/var/www/html/source/app-sat/cron/processa_log.php");
                $dataLog = date('Y-m-d H:i:s');
                $appCallLog = 'E-mail automático - Loja Block';
                $msgLog = $msgLog . " sem necessidade de envio de e-mail."; 
                insert_log_III('99','System','root',$appCallLog,$dataLog,$msgLog);
            }
            ...
            if(isset($objDiasCuponsAculados)){
                ...
                foreach($objDiasCuponsAculados as $key=>$val){
                    ...
                    $nsat = $objDiasCuponsAculados[$key]->sat;
            ...
            if(isset($objCuponsAculados)){
                ...
                foreach($objCuponsAculados as $key=>$val){
                    ...
                    $nsat = $objCuponsAculados[$key]->sat;
            ...
            unset($body);
            $body = "Segue abaixo lista de SATs:<br /><br />";
            if(isset($objSATLojaBlock)){
                foreach($objSATLojaBlock as $key=>$val){
                    $body = $body . "# Número SAT: " . $objSATLojaBlock[$key]->sat . " Loja: " . $objSATLojaBlock[$key]->loja . " Caixa: " . $objSATLojaBlock[$key]->caixa . " Estado WAN: " . $objSATLojaBlock[$key]->status_wan . " Data/Hora Comun. Sefaz: ". $objSATLojaBlock[$key]->data_hora_comun_sefaz ."<br />";
                }
                if (isset($objListEmailAlerta)) {
                    foreach ($objListEmailAlerta as $key=>$val) {
                        send_email_alerta(99, $objListEmailAlerta[$key]->email, 'root', 'System', $body, 'Loja Bloqueio');
                    }
                }else{
                    require_once("/var/www/html/source/app-sat/cron/processa_log.php");
                    $dataLog = date('Y-m-d H:i:s');
                    $appCallLog = 'E-mail automático - Lista E-mail Alertas';
                    $msgLog = "Erro ao verificar lista de e-mails alertas.";  
                    insert_log_III('99','System','root',$appCallLog,$dataLog,$msgLog);
                }
                if (isset($objListEmailExceptionAlerta)) {
                    foreach ($objListEmailExceptionAlerta as $key=>$val) {
                        send_email_alerta(99, $objListEmailExceptionAlerta[$key]->email, 'root', 'System', $body, 'Loja Bloqueio');
                    }
                }else{
                    require_once("/var/www/html/source/app-sat/cron/processa_log.php");
                    $dataLog = date('Y-m-d H:i:s');
                    $appCallLog = 'E-mail automático - Lista E-mail Exception';
                    $msgLog = "Erro ao verificar lista de e-mails de exceção.";  
                    insert_log_III('99','System','root',$appCallLog,$dataLog,$msgLog);
                }
                unset($msgLog);
                $msgLog = "SATs ";
                foreach($objSATLojaBlock as $key=>$val){
                    $msgLog = $msgLog . ",[" . $objSATLojaBlock[$key]->sat . "]";
                    $nsat = $objSATLojaBlock[$key]->sat;
                    $insertSendEmail = "INSERT INTO tb_send_email_auto (sat,motivo_email,data_hora_envio) VALUES ('$nsat','ESTADO_BLOCK_LOJA',NOW())";
                    $queryInsertSendEmail = mysqli_query($conn,$insertSendEmail);
                    if(mysqli_insert_id($conn)){
                        require_once("/var/www/html/source/app-sat/cron/processa_log.php");
                        $dataLog = date('Y-m-d H:i:s');
                ...
                $dataLog = date('Y-m-d H:i:s');
                $appCallLog = 'E-mail automático - Loja Block';
                $msgLog = "Nenhuma loja com bloqueio iminente."; 
                insert_log_III('99','System','root',$appCallLog,$dataLog,$msgLog); 
            }
        }else{
            require_once("/var/www/html/source/app-sat/cron/processa_log.php");
