<?php
    ...
    if ($id != $idUserActual  && $_SESSION['usuarioNivelDashSAT'] == 1 || $_SESSION['usuarioNivelDashSAT'] == 2) {
        if($_SESSION['usuarioNivelDashSAT'] != 1){
            ...
            $appCallLog = 'Lock Usuário'; 
            $msgLog = 'Lock ['.$id.']:['.$login.']:['.$nivel.'] senha.Ops!Usuário não permitido.';
            ...
            if ($rowExisteBloqueio['existe_registro'] >= 1) {
                ...
                $sqlUpdateBloqTemp = "UPDATE tb_bloqueio_temporario SET count_tentativas = 5 WHERE id = '$idTempBloqUser'";
                $queryUpdateBloqTemp = mysqli_query($conn, $sqlUpdateBloqTemp);
                if (mysqli_affected_rows($conn)) {
                    $dataLog = date('Y-m-d H:i:s');
                    $appCallLog = 'Lock Usuário';
                    $msgLog = 'Usuário ['.$login.'] já possui um bloqueio temporário ativo, foi atualizado o número tentativas [5].';
                    if ($_SESSION['usuarioIDDashSAT'] != 0) {
                    ...
                    if (mysqli_affected_rows($conn)) {
                        $dataLog = date('Y-m-d H:i:s');
                        $appCallLog = 'Lock Usuário';
                        $msgLog = 'Usuário ['.$login.'] bloqueado com sucesso.';
                        if ($_SESSION['usuarioIDDashSAT'] != 0) {
                            insert_log($_SESSION['usuarioIDDashSAT'], $_SESSION['usuarioNomeDashSAT'], $_SESSION['usuarioLoginDashSAT'], $appCallLog, $dataLog, $msgLog);
                        }
                        if($_SESSION['usuarioNivelDashSAT'] == 1){
                            echo '<script>alert("Usuário ['.$login.'] bloqueado com sucesso!");</script>';
                            echo '<meta HTTP-EQUIV="Refresh" CONTENT="0; URL=../superadmin/configuracao.php?token='.$token.'">';
                        }elseif($_SESSION['usuarioNivelDashSAT'] == 2){
                            echo '<script>alert("Usuário ['.$login.'] bloqueado com sucesso!");</script>';
                            echo '<meta HTTP-EQUIV="Refresh" CONTENT="0; URL=../admin/configuracao.php?token='.$token.'">';
                        ...
                        $dataLog = date('Y-m-d H:i:s');
                        $appCallLog = 'Lock Usuário';
                        $msgLog = 'Usuário ['.$login.'] nenhuma alteração no bloqueio.';
                        if ($_SESSION['usuarioIDDashSAT'] != 0) {
                            insert_log($_SESSION['usuarioIDDashSAT'], $_SESSION['usuarioNomeDashSAT'], $_SESSION['usuarioLoginDashSAT'], $appCallLog, $dataLog, $msgLog);
                        }
                        if($_SESSION['usuarioNivelDashSAT'] == 1){
                            echo '<script>alert("Usuário ['.$login.'] nenhuma alteração no bloqueio!");</script>';
                            echo '<meta HTTP-EQUIV="Refresh" CONTENT="0; URL=../superadmin/configuracao.php?token='.$token.'">';
                        }elseif($_SESSION['usuarioNivelDashSAT'] == 2){
                            echo '<script>alert("Usuário ['.$login.'] nenhuma alteração no bloqueio!");</script>';
                            echo '<meta HTTP-EQUIV="Refresh" CONTENT="0; URL=../admin/configuracao.php?token='.$token.'">';
                    ...
                    $dataLog = date('Y-m-d H:i:s');
                    $appCallLog = 'Lock Usuário';
                    $msgLog = 'Usuário ['.$login.'] já possui um bloqueio temporário ativo, nenhuma alteração realizada.';
                    if ($_SESSION['usuarioIDDashSAT'] != 0) {
                        insert_log($_SESSION['usuarioIDDashSAT'], $_SESSION['usuarioNomeDashSAT'], $_SESSION['usuarioLoginDashSAT'], $appCallLog, $dataLog, $msgLog);
                    }
                    $sqlUpdateUsrBloqTemp = "UPDATE tb_usuarios_dashsat SET ativo = 3 WHERE id = '$id'";
                ...
                if (mysqli_insert_id($conn)) {
                    ...
                    $dataLog = date('Y-m-d H:i:s');
                    $appCallLog = 'Lock Usuário';
                    $msgLog = 'Usuário ['.$login.'] erro ao bloquear usuário.';
                    if ($_SESSION['usuarioIDDashSAT'] != 0) {
                        insert_log($_SESSION['usuarioIDDashSAT'], $_SESSION['usuarioNomeDashSAT'], $_SESSION['usuarioLoginDashSAT'], $appCallLog, $dataLog, $msgLog);
                    }
                    if($_SESSION['usuarioNivelDashSAT'] == 1){
                        echo '<script>alert("Usuário ['.$login.'] erro ao bloquear usuário.");</script>';
                        echo '<meta HTTP-EQUIV="Refresh" CONTENT="0; URL=../superadmin/configuracao.php?token='.$token.'">';
                    }elseif($_SESSION['usuarioNivelDashSAT'] == 2){
                        echo '<script>alert("Usuário ['.$login.'] erro ao bloquear usuário.");</script>';
                        echo '<meta HTTP-EQUIV="Refresh" CONTENT="0; URL=../admin/configuracao.php?token='.$token.'">';
