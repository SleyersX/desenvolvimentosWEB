                    ...
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        ...
                        <li class="nav-item has-treeview menu-open">
                            ...
                            <ul class="nav nav-treeview">
                                ...
                                <li class="nav-item">
                                    <a href="tb-sats-inativos.php?token=<?php echo $token;?>" class="nav-link active">
                                        <i class="fas fa-wifi-slash nav-icon"></i>
                                        <p>S&#64;Ts Inativos</p>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="tb-shops-closed.php?token=<?php echo $token;?>" class="nav-link">
            ...
            <div class="content-wrapper">
                ...
                <section class="content-header">
                    <div class="container-fluid">
                        <div class="row mb-2">
                            <div class="col-sm-6">
                                <h1>Relação de S&#64;Ts Inativos/Desativados das lojas</h1>
                            </div>
                            <div class="col-sm-6">
                                <ol class="breadcrumb float-sm-right">
                                    <li class="breadcrumb-item"><a href="../home.php?token=<?php echo $token;?>">Home</a></li>
                                    <li class="breadcrumb-item active">Relação de S&#64;Ts Inativos/Desativados das lojas</li>
                                </ol>
                            </div>
                        </div>
                    </div><!-- /.container-fluid -->
                ...
                <section class="content">
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="card">
                                <div class="card-header">
                                    <h3 class="card-title">Relação de S&#64;Ts Inativos/Desativados das lojas</h3>
                                    <div class="card-tools">
                                    </div>
                                    <ol class="float-sm-right">
                                        <button id="historicoSat" type="button" class="btn-block btn-outline-info btn-sm">
                                            Exibir historico
                                        </button>
                                    </ol>
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body">
                                    <h6 class="text-sm text-left text-red">*Selectione uma linha na tabela para exibir o histórico</h6>
                                    <div class="table-responsive-sm">
                                        <table id="tbDadosSatsInativos" class="table table-bordered table-hover">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>S&#64;T</th>
                                            ...
                                            <?php
                                                $sqlSatDadosInativo = "SELECT sat FROM ". DATA_CONFIG_BD["cn_group_sats_inativo"] ."";
                                                $querySatDadosInativo = mysqli_query($conn,$sqlSatDadosInativo);
                                            ?>
                                            <tbody>
                                            <?php
                                                $i=1;
                                                while($resultSatDadosInativo = mysqli_fetch_array($querySatDadosInativo)){
                                                    $id             = $i;
                                                    $numSat         = $resultSatDadosInativo['sat'];
                                            ?>    
                                                    <tr id="trTbSatsInativos">
                                                        <td><?php echo $id?></td>
                                                        <td><?php echo $numSat?></td>
                                                    </tr>
                                            <?php
                                                    $i++;
                                                }
                        ...
                        <div class="col-sm-6" style="display: none;" id="div-history-sat">
                            <div class="card" data-card-widget="card" data-source="tb-sats-inativos.php?token=<?php echo $token;?>" data-source-selector="#card">
                                <div class="card-header">
                                    <h3 class="card-title">History S@T</h3>
                                    <div class="card-tools">
                                    </div>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive-sm">
                                        <table id="tbHistorySat" class="table table-bordered table-hover table-sm">
                                            <thead>
                                                <tr>
                                                    <th>S@T</th>
                                                    <th>Data Inclusão</th>
        ...
        <script>
            $(function () {
                $('#tbDadosSatsInativos').DataTable({
                    ...
                    "pageLength": 5,
                    "buttons": ["excel"]
                }).buttons().container().appendTo('#tbDadosSatsInativos_wrapper .col-md-6:eq(0)');
            });
        </script>
        <script type="text/javascript">
                var tabela = document.getElementById("tbDadosSatsInativos");
                ...
                var btnVisualizar = document.getElementById("historicoSat");
                btnVisualizar.addEventListener("click", function(){
                    ...
                    for(var i = 0; i < selecionados.length; i++){
                        ...
                        nsat = selecionado[1].innerHTML;
                    }

                    $('#div-history-sat').css("display","block")
                    $('#tbodyHistorySat').empty();
                    $.ajax({
                        url: "../../processo/get_history_sat.php",
                        type: "POST",
                        dataType: "json",
                        data: {sat:nsat},
                        success: function(data){
                            if(data){
                                for(var i=0;data.length>i;i++){
                                    $('#tbodyHistorySat').append('<tr><td><a href="../getSatInativo.php?token=<?php echo $token;?>&idsat='+data[i].id+'">'+data[i].sat+'</a></td><td>'+data[i].data_inclusao+'</td><td>'+data[i].shop+'</td><td>'+data[i].pos+'</td><td>'+data[i].data_atualizacao+'</td><td>'+data[i].status_atual+'</td></tr>');
                                }
                                $('#tbHistorySat').DataTable({
                                    "retrieve": true,
                                    "reponsive": true,
                                    "paging": true,
                                    "lengthChange": false,
