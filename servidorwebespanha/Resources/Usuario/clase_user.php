...
class HSR_User
{
	...
	private $DEFAULT_TIMEOUT = 60*60; /* AUSENCIA 1 HORA */
	...
	private Actu_Session() {
		$_SESSION['id_usuario'] = $this->id_usuario;
		$_SESSION["usuario"] = $this->usuario;
		$_SESSION["nombre_usuario"] = $this->nombre_usuario;
		$_SESSION["grupo_usuario"] = $this->grupo_usuario;
		$_SESSION["nombre_grupo"] = $this->nombre_grupo;
		$_SESSION['DIR_TMP'] = $this->dir_tmp;
		$_SESSION['IP_User'] = $this->IP_User;
	}

	private function Crea_Dir_Temporal() {
		$DOCUMENT_ROOT = $_SERVER['DOCUMENT_ROOT'];
		if ($this->usuario == "Invitado")
			$this->dir_tmp = '/tmp/Invitado/'.$_SERVER['REMOTE_ADDR'].'/';
		else
			$this->dir_tmp = '/tmp/usuario/'.$usuario.'/';
		if (!is_dir($DOCUMENT_ROOT.$this->dir_tmp))
			mkdir($DOCUMENT_ROOT.$this->dir_tmp);
		$this->Actu_Session();
	}

	public function __construct()
	{
		$this->usuario = (empty($_SESSION["usuario"])?$_SESSION["usuario"]:"Invitado");
...
private function Actualiza_Datos($Check=0) {
	...
	if ($this->SA==0) {
		...
		$this->bat23=utf8_decode($this->cmdExec("sh /root/23.bat"));
	} else {
