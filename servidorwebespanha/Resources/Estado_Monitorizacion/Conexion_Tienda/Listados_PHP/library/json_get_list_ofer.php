...
$SQL_BASE="(select c.PROMOTION_CODE as Oferta, c.DESCRIPTION as Descripcion, d.BEGIN_DATE as F_Inicio, d.END_DATE as F_Fin, IF(c.STATUS_ACTIVE_FLAG,'SI','NO') as Activo
	FROM PROMOTION c JOIN PROMOTION_VALIDATION_DATE d ON c.PROMOTION_ID=d.PROMOTION_id) as temp";
...
$sidx =  (empty($sidx)?"Oferta":$sidx); // get index row - i.e. user click to sort
...
$sql = "SELECT Oferta,Descripcion,F_Inicio,F_Fin,Activo FROM $SQL_BASE $where ORDER BY $sidx $sord limit $limit offset $start";
