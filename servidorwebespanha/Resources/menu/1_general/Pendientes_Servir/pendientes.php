	...
	<table style="width:100%;">
		...
		<tr id="panel_1">
			...
			<td><div class="con_borde">
				<h2>Opciones:</h2>
				<table>
					<tr><td><label>Fecha inicio</label></td><td><input type="date" id="Fecha_inicio"></td></tr>
					<tr><td><label>Fecha fin</label></td><td><input type="date" id="Fecha_fin"></td></tr>
					<tr><td colspan="2"><button id="confirma_fechas">Confirmar fechas</button></td></tr>
				</table>
				<hr>
				<button id="export_csv">Exportar a CVS</button>
			</div></td>
		<tr>
	</table>
...
<script>
	...
	var Fecha_Inicio="<?php echo '2018-01-01'; ?>"; $("#Fecha_inicio").attr("value",Fecha_Inicio);
	var Fecha_Fin="<?php echo date('Y-m-d'); ?>"; $("#Fecha_fin").attr("value",Fecha_Fin);

	function Cambia_Fechas() {
		console.log(Fecha_Inicio,Fecha_Fin);
	}

	$("#confirma_fechas").on("click",function () {
		Fecha_Inicio=$("#Fecha_inicio").val();
		Fecha_Fin=$("#Fecha_fin").val();
		drawChartsPendientes();
	});
...
//		Graph_Tiendas.draw(view1, { width:"100%"});
	...
	function show_por_tienda(row) {
		...
		Lista_Tienda.sort(0);
		view2.setRows(view2.getFilteredRows([{column: 0, minValue: new Date(Fecha_Inicio), maxValue: new Date(Fecha_Fin)}]));
		var Tabla_Tienda = new google.visualization.Table(document.getElementById('lista_movim_tienda'));
		Tabla_Tienda.draw(view2);

		
		var graph_Tabla_Tienda = new google.visualization.LineChart(document.getElementById('graph_movim_tienda'));
		graph_Tabla_Tienda.draw(view2, { width:"100%", 'focusTarget': 'category'});
		

		if (row > -1) {
			$("#info_por_tienda").html("Informacion de las pendientes de servir, pedidos y entregas por cada articulo, de la tienda "+tienda+" desde el "+Fecha_Inicio+" hasta el "+Fecha_Fin+".");
