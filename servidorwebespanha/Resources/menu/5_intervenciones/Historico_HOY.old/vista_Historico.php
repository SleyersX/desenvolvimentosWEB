...
switch (@$_GET["opcion"]) {
	case "Resumen_Intervenciones":
		...
		$sql=myQUERY("select Comentario,count(*) from Historico where date(fecha)=CURRENT_DATE group by 1 order by 2 desc");
		if (count($sql)>0) {
			$tmp="<table id='t_resumen_historico_hoy' class='tabla2'><thead><tr><th>Acciones</th><th>Cantidad</th></tr></thead>";
			foreach($sql as $d) {
				$clase="";
				if (preg_match("/INTERVENCION/", $d[0])) $clase="Intervencion";
				if (preg_match("/INSTALACION/", $d[0])) $clase="Instalacion'";
				if (preg_match("/ERROR/", $d[0])) $clase="ERROR";
				if ($clase) {
					$tmp.="<tr class='$clase'><td>".$d[0]."</td><td>".$d[1]."</td></tr>";
				}
			}
			$tmp.="</table>";
		}
	...
	case "Historico":
		$start=(empty($_GET["start"])?0:$_GET["start"]); $limit=(empty($_GET["limit"])?25:$_GET["limit"]);
		$query="select
				  a.Tienda
				, a.Caja
				, c.Version
	...
	case "Actu_Futu":
		switch ($PAIS_SERVER) {
			...
			case "BRA":
				...
				$res1=file_get_contents("/home/soporteweb/tmp/Versiones_Tienda/checklist.dat");
...
switch ($PAIS_SERVER) {
	case "BRA": $url_actual=str_replace($_SERVER['DOCUMENT_ROOT'], "/", __FILE__); break;
	default: $url_actual=str_replace($_SERVER['DOCUMENT_ROOT'], "", __FILE__); break;
}
...
<div style='border:1px solid black; border-radius:2px;background-color:white;'>
	<table>
		...
		<tr>
			<td colspan="2"><div id='vista_resumen_historico'></div></td>
		</tr>
		<tr>
...
<script>
	...
	clearInterval(interval_resumen_historico_hoy); var interval_resumen_historico_hoy=en_background("#vista_resumen_historico", url_actual+'?opcion=Resumen_Intervenciones',10000);
