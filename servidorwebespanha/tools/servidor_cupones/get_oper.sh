...
case "$QUIEN_SOY" in
	...
	"171.100.1.145")
		...
		;;
	"10.105.186.135")
		SERVIDORES=(10.106.68.15 10.106.68.22)
		PRG="/opt/SC/bin/estadisticas_hoy.sh"
		echo "BRA" > pais.dat
		;;
...
case "$OPTION" in
	"GET_OPER")
		for serv in 1 2; do
			...
			[ ! -z "$PRG" ] && ssh -i /home/soporte/.ssh/id_cupones -lsoporte $SERVIDOR "$PRG" | sort -g > $FILE_OPER
			[ ! -z "$PRG_WEB" ] && ssh -i /home/soporte/.ssh/id_cupones -lsoporte $SERVIDOR "$PRG_WEB" | sort -g > $FILE_OPER_WEB
			...
			mysql soporteremotoweb -N -e "SELECT time(id),Oper FROM $TABLA WHERE date(id)=date(now()) AND time(id) BETWEEN '00:00:00' AND '23:50:00'" > $FILE_DATOS.new
			...
			mysql soporteremotoweb -e "SELECT time(id),Web FROM $TABLA WHERE date(id)=date(now()) AND time(id) BETWEEN '00:00:00' AND '23:50:00'" | grep -v time > $FILE_DATOS_WEB.new
		done
		rm porhora*.jpg -f
		gnuplot ./crea.gp
		gnuplot ./crea_web.gp

		bash crea_tabla_hoy.sh
