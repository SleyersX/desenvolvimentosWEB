<?php
    ...
    require_once "/var/www/html/restapi/cron/controllers/config_email_auto.php";
        ...
        if($rowCountBDSAT['total_registros'] >= 1){
            ...
            while($rowBDSAT = mysqli_fetch_array($queryBDSAT)){
                ...
                if($comunicacaoSefaz == 1){
                    $sqlVerificaSATComunicacaoSefaz = "SELECT COUNT(sat) AS existe_sat, sat, loja, caixa, DATE_FORMAT(data_hora_comun_sefaz,'%d/%m/%Y - %H:%i:%s') AS data_hora_comun_sefaz, n_dias FROM cn_hml_sat_data_comun_sefaz WHERE sat = '$sat'";
                    ...
                    if($rowVerificaSATComunicacaoSefaz['existe_sat'] >= 1){
                        if($rowVerificaSATComunicacaoSefaz['n_dias'] >= $diasSemComunicarfSefaz){
