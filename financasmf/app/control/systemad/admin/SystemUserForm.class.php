...
class SystemUserForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $unit_id             = new TDBCombo('system_unit_id', _PERMISSION_, 'SystemUnit','id','name');
        $program_id          = new TDBSeekButton('program_id', _PERMISSION_, 'form_System_user', 'SystemProgram', 'name', 'program_id', 'program_name');
        $program_name        = new TEntry('program_name');
        $groups              = new TDBCheckGroup('groups', _PERMISSION_, 'SystemGroup','id','name');
        $frontpage_id        = new TDBUniqueSearch('frontpage_id', _PERMISSION_, 'SystemProgram', 'id', 'name', 'name');
        $units               = new TDBCheckGroup('units', _PERMISSION_, 'SystemUnit','id','name');
        ...
        $repassword->setMaxLength(20);
        
        $this->form->addFields( [new TLabel('ID')], [$id],  [new TLabel(_t('Name'),'red')], [$name] );
        $this->form->addFields( [new TLabel(_t('Login'),'red')], [$login],  [new TLabel(_t('Email'), 'red')], [$email] );
        ...
        $this->form->addFields( [new TFormSeparator('Grupos de Permissões', 'blue')] );
    ...
    public static function onSave($param)
    {
        try
        {
            ...
            TTransaction::open(_PERMISSION_);
            ...
            (new TRequiredValidator)->validate('Nome', $object->name);
            (new TRequiredValidator)->validate('Login', $object->login);
            (new TRequiredValidator)->validate('Email', $object->email);
            (new TEmailValidator)->validate('Email', $object->email);
