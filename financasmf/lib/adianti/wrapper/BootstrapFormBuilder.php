...
use Adianti\Widget\Util\TDropDown;  // ALV - dropdown
...
class BootstrapFormBuilder implements AdiantiFormInterface
{
    ...
    //-----------------------------------------------
    // ALV
    public function addHeaderDropDownAction(TDropDown $drop, $hint='', $icon = 'fa:gears blue')
    {
        $drop->{'title'} = $hint;
        $drop->setPullSide('right');
        $this->header_actions[] = $drop;
        return $drop;
    }

    public function addDropDownAction(TDropDown $drop, $hint='', $icon = 'fa:gears blue')
    {
        $drop->{'title'} = $hint;
        $drop->setPullSide('left');
        $this->actions[] = $drop;
        return $drop;
    }
    //-----------------------------------------------
